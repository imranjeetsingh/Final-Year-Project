...
file_ans = open("label_word_xml_landmark_train.txt", "w")
...
#     print(image_no, segment_no, word_list_match_no)
    ...
    for word in file_word_list:
        ...
        if str(word_index) == str(word_list_match_no):
            ...
            file_xml_data = open("Landmark_data.txt", "r")
            # traj_detail = 0
            for img in file_xml_data:
                xml_img_no = img.split('\t', 1)[0]
                xml_word1 = img.split('\t', 1)[1].strip()
                xml_word = xml_word1.split('\t',1)[0]
                ...
                if str(image_no) == str(xml_img_no):
                    ...
                    if(similarity1>=similarity):
                        ...
                        sen_no      = xml_word1.split('\t',1)[1]
    if similarity>=0.20:
        file_ans.write(image_no+'\t'+segment_no+'\t'+str(word_detail)+'\t'+str(sen_no)+'\t'+str(similarity)+'\t'+str("1")+'\n')
    else:
