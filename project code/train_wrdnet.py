...
file_ans = open("new_label_word_train.txt", "w")
...
for line in file_labels:
    ...
    segment_no = line.split('\t')[1].strip()
    word_list_match_no = line.split('\t')[2].strip()
#     print(image_no, segment_no, word_list_match_no)
    ...
    # word_match  = str()
    for word in file_word_list:
        word_index = word.split('\t', 1)[0].strip()
        word_match = word.split('\t', 1)[1].strip()
        if str(word_index) == str(word_list_match_no):
            # file_ans.write(word_match+'\t')
            file_xml_data = open("trainewdata.txt", "r")
            # traj_detail = 0
            for img in file_xml_data:
                xml_img_no = img.split('\t')[0].strip()
                xml_sen = img.split('\t')[1].strip()
                xml_word = img.split('\t')[2].strip()
                ...
                if str(image_no) == str(xml_img_no):
                    ...
                    #     else:
                    #         word2 = word2+w
                    similarity1 = sentence_similarity(word1, xml_word)
                    if(similarity1>=similarity):
                        ...
                        word11 = word1
    if similarity>=0.20:
        file_ans.write(str(image_no)+'\t'+str(segment_no)+'\t'+str(word11)+'\t'+str(word_detail)+'\t'+str(sen_no)+'\t'+str(similarity)+'\n')
    else:
        
        # file_ans.write(image_no+'\t'+segment_no+'\t'+str(word_detail)+'\t'+str(sen_no)+'\t'+str(similarity)+'\t'+str("0")+'\n')
